...
def FullOTA_InstallEnd(info):
    ...
    info.script.AppendExtra('mount("ext4", "EMCC", "/dev/block/platform/13500000.dwmmc0/by-name/VENDOR", "/vendor");');
    info.script.AppendExtra('run_program("/sbin/sh", "/system/system/bin/device_check.sh");');
    info.script.AppendExtra('unmount("/system");');
    info.script.AppendExtra('unmount("/vendor");');

def FullOTA_PostValidate(info):
    info.script.AppendExtra('run_program("/sbin/e2fsck", "-fy", "/dev/block/platform/13500000.dwmmc0/by-name/SYSTEM");');
    info.script.AppendExtra('run_program("/sbin/resize2fs", "/dev/block/platform/13500000.dwmmc0/by-name/SYSTEM");');
    info.script.AppendExtra('run_program("/sbin/e2fsck", "-fy", "/dev/block/platform/13500000.dwmmc0/by-name/SYSTEM");');
